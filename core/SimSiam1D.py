...
class SimSiam1DLearner:
    ...
    def run(self, train_dataset, val_dataset, test_dataset):
        ...
        self.logger.info("Executing SimSiam")
    ...
    def main_worker(self, rank, world_size, train_dataset, val_dataset, test_dataset, logs):
        ...
        if self.cfg.mode == 'pretrain' or self.cfg.mode == 'eval_pretrain':
            net = SimSiamNet(self.cfg.input_channels, self.cfg.z_dim, self.cfg.out_dim, self.cfg.pred_dim, self.cfg.mlp)
        elif self.cfg.mode == 'finetune' or self.cfg.mode == 'eval_finetune':
            net = SimSiamClassifier(self.cfg.input_channels, self.cfg.z_dim, self.cfg.num_cls, self.cfg.mlp)
            ...
            if self.cfg.neg_per_domain:
                ...
                for dom in self.all_domains:
                    ...
                    if torch.numel(dom_idx):
                        ...
                        p1, p2, z1, z2 = net(dom_features, dom_pos_features)
                        p1s.append(p1)
                        p2s.append(p2)
                        z1s.append(z1)
                        z2s.append(z2)
