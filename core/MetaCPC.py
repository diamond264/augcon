...
class Predictor(nn.Module):
    def __init__(self, z_dim=256, pred_steps=12):
        ...
        predictor = nn.ConvTranspose2d(z_dim, z_dim, (1, pred_steps))
...
class CPCNet(nn.Module):
    ...
    def sample_negatives(self, y, n_negatives, bsz):
        ...
        with torch.no_grad():
            ...
            if n_negatives > 0:
                tszs = (
                    buffered_arange(tsz).unsqueeze(-1).expand(-1, n_negatives).flatten()
                )
        ...
        negs = negs.view(fsz, bsz, n_negatives, tsz).permute(2, 1, 0, 3)  # to NxBxCxT
        return negs
...
class CPCClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.pooling == "mean":
            ...
            raise ValueError(
                "Invalid pooling mode. Please choose from 'mean', 'max', or 'sum'."
            )
            ...
            if not os.path.isfile(self.cfg.resume):
                ...
                if self.cfg.domain_adaptation:
                    ...
                    enc_parameters = self.meta_train(
                        rank, net, support, criterion, log_internals=True, logs=logs
                    )
                else:
        ...
        with torch.no_grad():
            if num_task is None:
                ...
                for _ in range(num_task):
                    ...
                    # support = torch.utils.data.Subset(dataset, indices[:task_size])
                    support = torch.utils.data.Subset(
                        dataset, indices[: len(indices) // 2]
                    )
                    support = [e[0] for e in support]
                    if len(support) >= task_size:
                        support = random.sample(support, task_size)
                    else:
                        support = support * (task_size // len(support)) + random.sample(
                    ...
                    if len(query) >= task_size:
                        ...
                        query = query * (task_size // len(query)) + random.sample(
