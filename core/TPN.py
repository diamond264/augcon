...
class TPNNet(nn.Module):
    ...
    def forward(self, x):
        ...
        noise_y = self.noise_head(z)
        scale_y = self.scale_head(z)
        rotate_y = self.rotate_head(z)
        negate_y = self.negate_head(z)
        flip_y = self.flip_head(z)
        permute_y = self.permute_head(z)
        time_warp_y = self.time_warp_head(z)
        channel_shuffle_y = self.channel_shuffle_head(z)
...
class TPNLearner:
    ...
    def main_worker(self, rank, world_size, train_dataset, val_dataset, test_dataset, logs):
        ...
        if self.cfg.mode == 'pretrain' or self.cfg.mode == 'eval_pretrain':
            ...
            net = TPNClassifier(self.cfg.input_channels, self.cfg.z_dim, self.cfg.num_cls, self.cfg.mlp)
