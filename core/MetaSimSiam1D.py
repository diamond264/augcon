...
class Classifier(nn.Module):
    def __init__(self, in_dim=96, hidden_1=128, hidden_2=128, out_dim=96):
    ...
    def forward(self, x, vars=None, bn_training=True):
        ...
        x = F.batch_norm(
            x, running_mean, running_var, vars[1], vars[2], training=bn_training
        )
        x = F.relu(x, True)
        ...
        x = F.batch_norm(
            x, running_mean, running_var, vars[4], vars[5], training=bn_training
        )
        x = F.relu(x, True)
...
class SimSiamNet(nn.Module):
    ...
    def forward(self, x1, x2, vars=None, bn_training=True):
        ...
        if self.mlp:
            cls_vars = vars[
                len(self.encoder.parameters()) : -len(self.predictor.parameters())
            ]
...
class MetaSimSiam1DLearner:
                ...
                if self.cfg.mode == "pretrain":
                    ...
                    if self.cfg.multi_cond_num_task > 0:
                        (
                            ...
                            multi_cond_support_labels,
                            multi_cond_query_labels,
            ...
            if num_task is None:
                ...
                for _ in range(num_task):
                    ...
                    if len(support) >= task_size:
                        ...
                        support_label = (
                            support_label * (task_size // len(support_label))
                            + support_label[: task_size % len(support_label)]
                        )
                    support = torch.stack(support, dim=0)
                    pos_support = torch.stack(pos_support, dim=0)
                    ...
                    if len(query) >= task_size:
                        ...
                        query_label = (
                            query_label * (task_size // len(query_label))
                            + query_label[: task_size % len(query_label)]
                        )
                    query = torch.stack(query, dim=0)
                    pos_query = torch.stack(pos_query, dim=0)
    ...
    def pretrain(
        ...
        for task_idx in range(len(supports)):
            ...
            log_steps = False
            if epoch % self.cfg.log_freq == 0 and self.cfg.log_meta_train:
                log_steps = True
            ...
            q_loss = (
                -(criterion(q_p1, q_z2).mean() + criterion(q_p2, q_z1).mean()) * 0.5
            )
            ...
            if epoch % self.cfg.log_freq == 0:
                ...
                KNNCls.fit(
                    q_z1.detach().cpu().numpy()[: len(q_z1) // 2],
                    y=query_label[: len(q_z1) // 2].numpy(),
                )
                KNN_pred = KNNCls.predict(q_z1.detach().cpu().numpy()[len(q_z1) // 2 :])
                KNN_acc = (
                    np.mean(KNN_pred == query_label.numpy()[len(q_z1) // 2 :]) * 100
                )
                std = torch.std(
                    torch.cat(
                        (F.normalize(q_z1, dim=1), F.normalize(q_z1, dim=1)), dim=0
                    )
                )
    ...
    def meta_train(
        ...
        for i in range(self.cfg.task_steps):
            s_p1, s_p2, s_z1, s_z2 = net(
                support, pos_support, fast_weights, bn_training=bn_training
            )
            s_loss = (
                -(criterion(s_p1, s_z2).mean() + criterion(s_p2, s_z1).mean()) * 0.5
            )
            ...
            if log_steps and rank == 0:
                KNNCls = KNN(n_neighbors=3)
                KNNCls.fit(
                    s_z1.detach().cpu().numpy()[: len(s_z1) // 2],
                    y=support_label[: len(s_z1) // 2].numpy(),
                )
                KNN_pred = KNNCls.predict(s_z1.detach().cpu().numpy()[len(s_z1) // 2 :])
                KNN_acc = (
                    np.mean(KNN_pred == support_label.numpy()[len(s_z1) // 2 :]) * 100
                )
                std = torch.std(
                    torch.cat(
                        (F.normalize(s_z1, dim=1), F.normalize(s_z1, dim=1)), dim=0
                    )
                )
