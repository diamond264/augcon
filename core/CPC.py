...
class Aggregator(nn.Module):
    def __init__(self, num_blocks=5, num_filters=256, residual_scale=0.5):
        ...
        for i in range(num_blocks):
            ...
            relu_layer = nn.ReLU()
            self.blocks.append(
                nn.Sequential(zero_pad, conv_layer, norm_layer, relu_layer)
            )
...
class Predictor(nn.Module):
    def __init__(self, z_dim=256, pred_steps=12):
        super().__init__()
        self.predictor = nn.ConvTranspose2d(z_dim, z_dim, (1, pred_steps))
    ...
    def main_worker(
        ...
        if world_size > 1:
            ...
            self.write_log(
                rank,
                logs,
                "DDP is used for training - training {} instances for each worker".format(
        ...
        # Single GPU setting
            ...
            self.write_log(
                rank,
                logs,
                "Single GPU is used for training - training {} instances for each worker".format(
                    len(train_dataset)
                ),
            )
        ...
        if self.cfg.mode == "finetune" or self.cfg.mode == "eval":
            if os.path.isfile(self.cfg.pretrained):
                self.write_log(
                    rank,
                    logs,
                    "Loading pretrained model from checkpoint - {}".format(
                        self.cfg.pretrained
                    ),
                )
