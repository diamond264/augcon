...
def time_segment_permutation_transform_vectorized(X, num_segments=4, minSegLength=10):
    ...
    while bWhile == True:
        ...
        segs[1:-1] = np.sort(
            np.random.randint(minSegLength, X.shape[1] - minSegLength, num_segments - 1)
        )
        segs[-1] = X.shape[1]
...
def GenerateRandomCurves(X, sigma=0.2, knot=4):
    xx = (
        np.ones((X.shape[0], 1))
        * (np.arange(0, X.shape[1], (X.shape[1] - 1) / (knot + 1)))
    ).transpose()
    ...
    for i in range(X.shape[0]):
        cs = scipy.interpolate.CubicSpline(xx[:, i], yy[:, i])
...
def DistortTimesteps(X, sigma=0.2):
    ...
    # t_scale = [
    #     (X.shape[1] - 1) / tt_cum[-1, 0],
    #     (X.shape[1] - 1) / tt_cum[-1, 1],
    #     (X.shape[1] - 1) / tt_cum[-1, 2],
    # ]
    t_scale = []
    for i in range(X.shape[0]):
        t_scale.append((X.shape[1] - 1) / tt_cum[-1, i])
    for i in range(X.shape[0]):
...
def tpn_scale(X, choice, scale_range=0.5, min_scale_diff=0.15):
    if choice == 1:
        low = 1 - scale_range
        high = 1 + scale_range
        scaling_factor = np.random.uniform(low=low, high=high, size=(X.shape[0]))
