...
PRETRAIN_HPS = {
    "wesad": {"lr": 0.001, "wd": 0.0, "bs": 1024},
    "ninaprodb5": {"lr": 0.001, "wd": 0.0, "bs": 1024},
}
    ...
    for dataset in DATASETS:
        ...
        for domain in domains:
            ...
            pretrain_config_path = (
                f"{CONFIG_PATH}/{dataset}/darling/pretrain/gpu{gpu}_{domain}.yaml"
            )
            ...
            pretrain_ckpt_path = f"{MODEL_PATH}/{dataset}/darling/pretrain/{domain}"
            ...
            for seed in [0, 1, 2, 3, 4]:
                for shot in [1, 2, 5, 10, 20]:
                    for freeze in [True, False]:
                        ...
                        finetune_config_path = f"{CONFIG_PATH}/{dataset}/darling/finetune/{shot}shot/{setting}/seed{seed}/gpu{gpu}_{domain}.yaml"
                        print(f"Generating {finetune_config_path}")

                        finetune_path = (
                            f"{data_path}{domain}/finetune/{shot}shot/target"
                        )
                        finetune_ckpt_path = f"{MODEL_PATH}/{dataset}/darling/finetune/{shot}shot/{setting}/seed{seed}/{domain}"
                        pretrained_path = (
