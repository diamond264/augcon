...
def gen_pretrain_config():
    for dataset in DATASETS:
        ...
        for domain in domains:
            port = 8367 + gpu
            pretrain_config_path = f"{CONFIG_PATH}/{dataset}/prev_{PRETEXT}/pretrain/gpu{gpu}_{domain}.yaml"
            ...
            pretrain_ckpt_path = (
                f"{MODEL_PATH}/{dataset}/prev_{PRETEXT}/pretrain/{domain}"
            )
            ...
            for seed in [0, 1, 2, 3, 4]:
                for shot in [1, 2, 5, 10, 20]:
                    for freeze in [True]:
                        ...
                        finetune_config_path = f"{CONFIG_PATH}/{dataset}/prev_{PRETEXT}/finetune/{shot}shot/{setting}/seed{seed}/gpu{gpu}_{domain}.yaml"
                        print(f"Generating {finetune_config_path}")

                        finetune_path = (
                            f"{data_path}{domain}/finetune/{shot}shot/target"
                        )
                        finetune_ckpt_path = f"{MODEL_PATH}/{dataset}/prev_{PRETEXT}/finetune/{shot}shot/{setting}/seed{seed}/{domain}"
                        pretrained_path = (
...
adapt_w_neg: {'false' if mode == 'pretrain' else 'false'}
