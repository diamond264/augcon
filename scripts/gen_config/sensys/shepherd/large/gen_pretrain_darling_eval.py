...
CONFIG_PATH = "/mnt/sting/hjyoon/projects/aaa/configs/sensys/shepherd/model_size"
MODEL_PATH = "/mnt/sting/hjyoon/projects/aaa/models/sensys/shepherd/model_size"
NUM_LAYERS = [6]
        ...
        for domain in domains:
            for num_layers in NUM_LAYERS:
                ...
                pretrain_config_path = (
                    f"{CONFIG_PATH}/{dataset}/darling/pretrain/num_layer{num_layers}/gpu{gpu}_{domain}.yaml"
                )
                ...
                pretrain_ckpt_path = f"{MODEL_PATH}/{dataset}/darling/pretrain/num_layer{num_layers}/{domain}"
                ...
                for seed in [0, 1, 2, 3, 4]:
                    for shot in [1, 2, 5, 10, 20]:
                        for freeze in [True, False]:
                            ...
                            finetune_config_path = f"{CONFIG_PATH}/{dataset}/darling/finetune/num_layer{num_layers}/{shot}shot/{setting}/seed{seed}/gpu{gpu}_{domain}.yaml"
                            print(f"Generating {finetune_config_path}")

                            finetune_path = (
                                f"{data_path}{domain}/finetune/{shot}shot/target"
                            )
                            finetune_ckpt_path = f"{MODEL_PATH}/{dataset}/darling/finetune/num_layer{num_layers}/{shot}shot/{setting}/seed{seed}/{domain}"
                            pretrained_path = (
