...
def run():
    gpu = 0
    domains = ["clipart", "infograph", "real"]
    for target in [1, 2, 3]:
        for source in [1, 2, 3]:
...
resume: ''
pretrained: ./temp/pretrain{source}/checkpoint_0099.pth.tar
# pretrained: ''
...
no_vars: true'''
            gpu += 1
            if gpu >= 8: gpu = 0
            file_path = os.path.join(config_path, f'SimSiam_DomainNet_finetune{source}_{target}.yaml')
...
no_vars: true'''
        gpu += 1
        if gpu >= 8: gpu = 0
        file_path = os.path.join(config_path, f'SimSiam_DomainNet_finetune_random_{target}.yaml')
        os.makedirs(os.path.dirname(file_path), exist_ok=True)
        with open(file_path, 'w') as f:
            f.write(config)
