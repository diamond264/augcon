...
class ProcessCapture24():
    def __init__(self, file_paths):
        split_ratio = 0.9
        split_idx = int(len(file_paths) * split_ratio)
        random.shuffle(file_paths)
        self.train_file_paths = file_paths[:split_idx]
        self.test_file_paths = file_paths[split_idx:]
...
if __name__ == '__main__':
    file_path = '/mnt/sting/chahh/data/data/ssl/ssl_capture_24/data'
    users = glob(os.path.join(file_path, '*', '*.npy'))
    unums = [int(u.split('/')[-1].split('.')[0].split('P')[1]) for u in users]
    base_out_dir = '/mnt/sting/hjyoon/projects/cross/Capture-24/augcon'
    
    dataset = ProcessCapture24(file_path)
